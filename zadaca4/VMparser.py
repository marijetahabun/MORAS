class Parser:
    ...
    def _parseLine(self, line, n):
        ...
        if l[0] == "push":
            if len(l) == 3:
                if (not(l[2].isdigit())) or (int(l[2])<0):
                    self._flag = False
                    Parser._error("Parser", n, "Argument is not nonegative integer")
                    return ""
                else:
            ...
            if l[0] == "label" and len(l) == 2:
                ...
                if l[1] not in self._listaLabela:
                    self._flag = False
                    Parser._error("Parser", n, "Label not defined")
                    return ""
                else:
                    return "//" + " ".join(l) + "\n" + self._goto(l[1], n)
